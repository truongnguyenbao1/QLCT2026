...
class PrinterService {
        ...
        build: (ctx) {
          ...
          return pw.Column(
            ...
            children: [
              ...
              _receiptRowWrap(
                'Phong:',
                invoice.roomNumber.isNotEmpty ? invoice.roomNumber : '-',
                font, fontBold,
              ),
              _receiptRowWrap(
                'Khach thue:',
                (invoice.tenantName != null && invoice.tenantName!.isNotEmpty)
                    ? invoice.tenantName!
                    : 'Chua co khach',
                font, font,
              ),
              ...
              // ── Chi tiết ──────────────────────────────────────────────
              pw.SizedBox(height: 4),
              _receiptItem('Tien phong',
                  AppFormatters.formatCurrency(invoice.rentAmount), font, fontBold),

              if (invoice.serviceAmount > 0)
                _receiptItem('Phi dich vu',
                    AppFormatters.formatCurrency(invoice.serviceAmount), font, fontBold),
              ...
              _receiptRow('  Chi so moi:', '${invoice.electricCurrReading.toStringAsFixed(0)} kWh', font, font),
              _receiptRow('  Tieu thu:', '${invoice.electricUnitsUsed.toStringAsFixed(0)} kWh', font, fontBold),
              _receiptRow(
                '  Don gia:',
                '${AppFormatters.formatCurrency(invoice.electricUnitPrice)}/kWh',
                font, font,
              ),
              _receiptItem('Tien dien',
                  AppFormatters.formatCurrency(invoice.electricAmount), font, fontBold),
              ...
              _receiptRow('  Chi so moi:', '${invoice.waterCurrReading.toStringAsFixed(0)} m3', font, font),
              _receiptRow('  Tieu thu:', '${invoice.waterUnitsUsed.toStringAsFixed(0)} m3', font, fontBold),
              _receiptRow(
                '  Don gia:',
                '${AppFormatters.formatCurrency(invoice.waterUnitPrice)}/m3',
                font, font,
              ),
              _receiptItem('Tien nuoc',
                  AppFormatters.formatCurrency(invoice.waterAmount), font, fontBold),
              ...
              pw.SizedBox(height: 4),
              _receiptRow('Han thanh toan:', AppFormatters.formatDate(invoice.dueDate), font, fontBold),
              _receiptRow('Trang thai:', invoice.status.displayName, font, font),
              ...
              // Feed paper: khoảng trắng cuối để dao cắt không đè nội dung
              // Tương đương ~5 dòng feed trên máy nhiệt (≈15mm)
  ...
  /// Row 2 cột có wrap: label trái, value xuống dòng nếu dài
  static pw.Widget _receiptRowWrap(
    String label,
    String value,
    pw.Font labelFont,
    pw.Font valueFont,
